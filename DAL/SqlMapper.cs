...
namespace DAL
{
    ...
    public class DapperPocoInfo
    {
        ...
        public IEnumerable<PropertyInfo> TypeProperties
        {
            get
            {
                this._typeProperties = this._typeProperties.Where(p => p.GetCustomAttribute(typeof(AutoAttribute)) == null || (p.GetCustomAttribute(typeof(AutoAttribute)) != null && p.GetCustomAttribute(typeof(KeyAttribute)) != null)).ToArray();
        ...
        public IEnumerable<PropertyInfo> KeyProperties
        {
            get
            {
                if (this._keyProperties.Count == 0)
                {
                    ...
                    {
                        this._defaultKeyPropeties = this._typeProperties.Where(p => p.GetCustomAttribute(typeof(KeyAttribute)) != null).ToArray();

                        if (this._defaultKeyPropeties.Count() == 0)
                        {
                            throw new Exception("表" + TableName + "未找到主键特性  [KeyAttribute]");
                        }
        ...
        private IEnumerable<PropertyInfo> _autoPropeties;
        /// <summary>  
        /// 获取自动编号列
        /// </summary>  
        /// <returns></returns>  
        public IEnumerable<PropertyInfo> AutoProperties
        {
            get
            {

                if (this._autoPropeties == null)
                {
                    this._autoPropeties = this._typeProperties.Where(p => p.GetCustomAttribute(typeof(AutoAttribute)) != null).ToArray();


                }
                return this._autoPropeties;
    ...
    public class MsSqlServerAdapter : ISqlAdapter
    {
        ...
        private string GetInsertSql(DapperPocoInfo dpi, IEnumerable<PropertyInfo> props)
        {
            ...
            if (dpi.IsUnWriteKey())
            {
                valueProps = this.GetExceptProps(props, dpi.AutoProperties);
            }
        ...
        public bool Insert<T>(IDbConnection connection, object entity, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            ...
            if (connection.Execute<T>(insertSql, entity, transaction, commandTimeout) > 0)
            {
                if (entityType.IsAssignableFrom(type) && dpi.IsUnWriteKey())
                {
                    ...
                    if (key.PropertyType != typeof(Guid) && key.GetCustomAttribute(typeof(AutoAttribute)) != null)
                    {
        ...
        public bool UpdateByKey<T>(IDbConnection connection, object entity, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            ...
            if (connection.Execute<T>(updateSql, entity, transaction, commandTimeout) > 0)
            {
                if (entityType.IsAssignableFrom(type) && dpi.IsUnWriteKey())
                {
                    PropertyInfo key = dpi.KeyProperties.First();
    ...
    /* 
     ...
     Home page: https://code.google.com/p/dapper-dot-net/ 
...
this IDbConnection cnn, string sql, Func<TFirst, TSecond, TThird, TFourth, TReturn> map, dynamic param = null, IDbTransaction transaction = null, bool buffered = true, string splitOn = "Id", int? commandTimeout = null, CommandType? commandType = null
        ...
        public static void PackListParameters(IDbCommand command, string namePrefix, object value)
        {
            ...
            // keep in mind SQL support F to 2000 params easily in sp_executesql, needing more is rare  
